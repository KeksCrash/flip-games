...
param(
    ...
    # Virtueller Monitor: erstellt temporaer ein Parsec VDD Display, UVNC capturt nur dieses
    [switch]$VirtualMonitor,

    # Optionaler Pfad zu einem bereits heruntergeladenen parsec-vdd Installer
    [string]$VddInstallerPath = ""
)
...
# Parsec VDD Zustand (wird in Remove-GeneratedFiles benoetigt)
$script:vddOemInfName  = $null
$script:vddInstalled   = $false
...
function Assert-FileHash {
    ...
    if($actual -ne $ExpectedHash) {
        throw "$Description hat eine unerwartete SHA256-Prüfsumme. Erwartet: $ExpectedHash, erhalten: $actual"
    }
}
...
function Stop-TempWinVnc {
    $procs = @(
        Get-CimInstance Win32_Process -Filter "Name='winvnc.exe'" -ErrorAction SilentlyContinue |
            Where-Object { $_.ExecutablePath -and $_.ExecutablePath.StartsWith($tempRoot,[StringComparison]::OrdinalIgnoreCase) }
    )
    foreach($p in $procs) { Invoke-CimMethod -InputObject $p -MethodName Terminate | Out-Null }
}
...
# ── Parsec VDD (Virtueller Monitor) ──────────────────────────────────────────

function Get-VddDownloadUrl {
    # Dynamisch aus GitHub Releases ermitteln
    try {
        [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
        $release = Invoke-RestMethod "https://api.github.com/repos/parsec-cloud/parsec-vdd/releases/latest" -ErrorAction Stop
        $asset   = $release.assets | Where-Object { $_.name -like "*.exe" -or $_.name -like "*.msi" } | Select-Object -First 1
        if($asset) { return $asset.browser_download_url }
    } catch { }
    # Bekannte Fallback-URL (parsec-vdd v0.45)
    return "https://github.com/parsec-cloud/parsec-vdd/releases/download/v0.45/parsec-vdd-setup.exe"
}

function Get-VddOemInfName {
    # Findet den oem*.inf Namen des installierten Parsec VDD Treibers
    $result = pnputil /enum-drivers 2>&1 | Out-String
    $blocks = $result -split "(?=Published Name\s*:)"
    foreach($block in $blocks) {
        if($block -match "parsec" -or $block -match "VDD" -or $block -match "Virtual Display") {
...
function Install-VirtualDisplayDriver {
    Write-Host "Virtueller Monitor: Lade Parsec VDD ..." -ForegroundColor Cyan

    $installer = $VddInstallerPath
    if(-not $installer -or -not (Test-Path -LiteralPath $installer)) {
        $url = Get-VddDownloadUrl
        $installer = Join-Path $work "parsec-vdd-setup.exe"
        New-Item -ItemType Directory -Path $work -Force | Out-Null
        [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
        Invoke-WebRequest -Uri $url -OutFile $installer -UseBasicParsing -ErrorAction Stop
        Write-Host "  Heruntergeladen: $url" -ForegroundColor Green
    }

    # Stille Installation (NSIS /S oder MSI /qn)
    $ext = [IO.Path]::GetExtension($installer).ToLower()
    if($ext -eq ".msi") {
        $proc = Start-Process msiexec -ArgumentList "/i","$installer","/qn","/norestart" -Wait -PassThru -WindowStyle Hidden
    } else {
        $proc = Start-Process $installer -ArgumentList "/S" -Wait -PassThru -WindowStyle Hidden
    }

    if($proc.ExitCode -notin @(0,3010)) {
        throw "Parsec VDD Installation fehlgeschlagen (Exitcode $($proc.ExitCode))."
    }

    $script:vddInstalled = $true
    Write-Host "  Parsec VDD installiert." -ForegroundColor Green

    # Warten bis virtueller Monitor erscheint (max 15s)
    ...
    do {
        ...
        $virtualScreen = $screens | Where-Object { -not $_.Primary } | Select-Object -First 1
    } until($virtualScreen -or (Get-Date) -ge $deadline)

    if(-not $virtualScreen) {
        # Treiber evtl. aktiv aber Monitor noch nicht aufgezaehlt -> weiter versuchen
        Write-Warning "Virtueller Monitor noch nicht sichtbar. Fortsetzen mit primary=0."
    } else {
        Write-Host "  Virtueller Monitor erkannt: $($virtualScreen.DeviceName)" -ForegroundColor Green
    }

    $script:vddOemInfName = Get-VddOemInfName
}

function Uninstall-VirtualDisplayDriver {
    if(-not $script:vddInstalled) { return }
    Write-Host "Virtueller Monitor: Entferne Parsec VDD ..." -ForegroundColor Cyan

    # Ueber Add/Remove Programs deinstallieren (stille Deinstallation)
    $uninstaller = Get-ItemProperty `
        "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*",
        "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*" `
        -ErrorAction SilentlyContinue |
        Where-Object { $_.DisplayName -like "*parsec*vdd*" -or $_.DisplayName -like "*Virtual Display*" } |
        Select-Object -First 1

    if($uninstaller -and $uninstaller.UninstallString) {
        $cmd = $uninstaller.UninstallString
        if($cmd -like "msiexec*") {
            $guid = ([regex]'\{[0-9A-Fa-f\-]+\}').Match($cmd).Value
            if($guid) {
                Start-Process msiexec -ArgumentList "/x","$guid","/qn","/norestart" -Wait -WindowStyle Hidden -ErrorAction SilentlyContinue
            }
        } else {
            Start-Process $cmd -ArgumentList "/S" -Wait -WindowStyle Hidden -ErrorAction SilentlyContinue
        }
    }

    # Treiber-INF entfernen falls gefunden
    if($script:vddOemInfName) {
        pnputil /delete-driver $script:vddOemInfName /uninstall /force 2>&1 | Out-Null
    }

    $script:vddInstalled = $false
    Write-Host "  Parsec VDD entfernt." -ForegroundColor Green
}
...
function Remove-GeneratedFiles {
    ...
    Uninstall-VirtualDisplayDriver
...
function New-UltraVncIni {
    ...
    # primary=0 + secondary=1 -> nur virtueller (sekundaerer) Monitor
    # primary=1 + secondary=0 -> nur Hauptmonitor (Standard)
    $capturesPrimary   = if($VirtualMonitor) { 0 } else { 1 }
    $capturesSecondary = if($VirtualMonitor) { 1 } else { 0 }
...
primary=$capturesPrimary
secondary=$capturesSecondary
rdpmode=0
...
function Get-OfficialPasswordTools {
    ...
    $x86 = $tools | Where-Object {
        $_.File.Name -eq "createpassword.exe"   -and $_.Architecture -eq "x86" -and $_.Hash -eq $knownHashes.CreatePasswordX86
    } | Select-Object -First 1
    $x64 = $tools | Where-Object {
        $_.File.Name -eq "createpassword64.exe" -and $_.Architecture -eq "x64" -and $_.Hash -eq $knownHashes.CreatePasswordX64
    } | Select-Object -First 1

    if(!$x86 -or !$x64) {
        throw "Das offizielle Archiv enthält nicht die erwarteten, unveränderten x86/x64-Passwortwerkzeuge."
    }
    return @{ x86 = $x86.File.FullName; x64 = $x64.File.FullName }
}
...
function Set-PasswordWithOfficialTool {
    ...
    try { & $localTool $PlainPassword; $exitCode = $LASTEXITCODE }
...
function Expand-AndConfigureUltraVnc {
    ...
    if($servers.Count -eq 0) { throw "Das UltraVNC-Archiv enthält keine winvnc.exe." }
    ...
    foreach($server in $servers) {
        ...
        if($architecture -notin @("x86","x64") -or $prepared.Contains($architecture)) { continue }
        ...
        $generatedHashes[$architecture] = Set-PasswordWithOfficialTool `
            -Directory $destination -Architecture $architecture `
            -PlainPassword $PlainPassword -Tools $Tools
    ...
    if($prepared.Count -eq 0) { throw "Keine verwendbare x86- oder x64-Version wurde gefunden." }
...
function Get-InteractiveUser {
    ...
    if(!$userName) { throw "Kein interaktiv angemeldeter Windows-Benutzer wurde gefunden." }
    return $userName
}
...
"@
    ...
    if(!$process) { throw "WinVNC wurde nicht im interaktiven Desktop gestartet." }
    ...
    if($VirtualMonitor) {
        Write-Host "Virtueller Monitor aktiv (primary=0, secondary=1)." -ForegroundColor Green
    }
...
if($Mode -eq "Remove") {
    ...
    Write-Host "Temporäre UltraVNC-Prozesse, Aufgabe, Dateien und VDD-Treiber wurden entfernt." -ForegroundColor Green
    return
}
...
try {
    ...
    # Virtuellen Monitor einrichten (vor UltraVNC-Konfiguration)
    if($VirtualMonitor) {
    ...
    if($Version -eq "1.8.2.4") {
        Assert-FileHash -LiteralPath $uvncArchive -ExpectedHash $knownHashes.UltraVNC_1824 -Description "UltraVNC-Archiv"
    } else {
        Write-Warning "Für UltraVNC $Version ist keine feste Archivprüfsumme hinterlegt."
    }

    Assert-FileHash -LiteralPath $passwordArchive -ExpectedHash $knownHashes.CreatePassword -Description "Offizielles createpassword-Archiv"
...
if($Mode -eq "Run") {
    $selectedArchitecture = if(
        [Environment]::Is64BitOperatingSystem -and $prepared -contains "x64"
    ) { "x64" } elseif($prepared -contains "x86") { "x86" } else {
